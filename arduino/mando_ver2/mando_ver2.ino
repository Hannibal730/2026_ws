...
#define ENCODER_FAULT_GUARD_ENABLE 1
#define ENCODER_FAULT_MAX_DELTA_PER_10MS 60L
#define ENCODER_FAULT_MIN_DELTA_COUNTS 4L
#define ENCODER_FAULT_MIN_DT_MS 1UL
#define ENCODER_FAULT_MAX_DT_MS 200UL
#define ENCODER_RESET_SYNC_DROP_SAMPLES 1
#define EFG_STATUS_OK 0
#define EFG_STATUS_BASELINE 1
#define EFG_STATUS_STALE_DT 2
#define EFG_STATUS_SPIKE_DROPPED 3
...
long encoderGuardedCount = 0;
long encoderGuardPrevRawCount = 0;
unsigned long encoderGuardPrevMs = 0;
long encoderStreamPrevCount = 0;
unsigned long encoderStreamPrevMs = 0;
bool encoderGuardReady = false;
uint16_t encoderFaultRejects = 0;
uint8_t encoderGuardStatus = 0;
uint8_t encoderStreamSyncDropsPending = 0;
...
static inline long absLong(long value) {
  return (value < 0) ? -value : value;
}

long encoderMaxDeltaForDt(unsigned long dtMs) {
  unsigned long scaled =
      (ENCODER_FAULT_MAX_DELTA_PER_10MS * dtMs + ENCODER_STREAM_INTERVAL_MS - 1) /
      ENCODER_STREAM_INTERVAL_MS;
  if (scaled < (unsigned long)ENCODER_FAULT_MIN_DELTA_COUNTS) {
    scaled = ENCODER_FAULT_MIN_DELTA_COUNTS;
  }
  return (long)scaled;
}

void resetEncoderFaultGuard(long rawCount, unsigned long nowMs) {
  encoderGuardedCount = rawCount;
  encoderGuardPrevRawCount = rawCount;
  encoderGuardPrevMs = nowMs;
  encoderStreamPrevCount = rawCount;
  encoderStreamPrevMs = nowMs;
  encoderGuardReady = true;
  encoderGuardStatus = EFG_STATUS_BASELINE;
  encoderStreamSyncDropsPending = ENCODER_RESET_SYNC_DROP_SAMPLES;
}

long updateEncoderFaultGuard(long rawCount, unsigned long nowMs) {
#if ENCODER_FAULT_GUARD_ENABLE
  if (!encoderGuardReady) {
    resetEncoderFaultGuard(rawCount, nowMs);
    return encoderGuardedCount;
  }

  unsigned long dtMs = nowMs - encoderGuardPrevMs;
  if (dtMs < ENCODER_FAULT_MIN_DT_MS) {
    return encoderGuardedCount;
  }

  if (dtMs > ENCODER_FAULT_MAX_DT_MS) {
    encoderGuardPrevRawCount = rawCount;
    encoderGuardPrevMs = nowMs;
    encoderGuardStatus = EFG_STATUS_STALE_DT;
    return encoderGuardedCount;
  }

  long deltaCount = rawCount - encoderGuardPrevRawCount;
  long maxDelta = encoderMaxDeltaForDt(dtMs);

  if (absLong(deltaCount) > maxDelta) {
    encoderGuardPrevRawCount = rawCount;
    encoderGuardPrevMs = nowMs;
    encoderGuardStatus = EFG_STATUS_SPIKE_DROPPED;
    if (encoderFaultRejects < 65535) ++encoderFaultRejects;
    return encoderGuardedCount;
  }

  encoderGuardedCount += deltaCount;
  encoderGuardPrevRawCount = rawCount;
  encoderGuardPrevMs = nowMs;
  encoderGuardStatus = EFG_STATUS_OK;
  return encoderGuardedCount;
#else
  (void)nowMs;
  encoderGuardedCount = rawCount;
...
void resetEncoderCount() {
  ...
  resetEncoderFaultGuard(0, now);
  encoderFaultRejects = 0;
...
void printEncoderCsv(long count) {
  ...
  if (now - encoderCsvPrevMs >= ENCODER_STREAM_INTERVAL_MS) {
    ...
    unsigned long sampleDtMs = now - encoderStreamPrevMs;
    if (sampleDtMs == 0) sampleDtMs = ENCODER_STREAM_INTERVAL_MS;
    long deltaCount = count - encoderStreamPrevCount;
    int potVal = analogRead(POTPin);
    encoderCsvPrevMs += ENCODER_STREAM_INTERVAL_MS;
    if (now - encoderCsvPrevMs >= ENCODER_STREAM_INTERVAL_MS) encoderCsvPrevMs = now;

    if (encoderStreamSyncDropsPending > 0) {
      encoderStreamPrevCount = count;
      encoderStreamPrevMs = now;
      --encoderStreamSyncDropsPending;

      Serial.print("ENC_SYNC,");
...
void setup() {
  ...
  encoderCsvPrevMs = encoderTimeZeroMs;
  resetEncoderFaultGuard((long)encoderCount, encoderTimeZeroMs);
...
void loop() {
  ...
  long raw_encoder_count = (long)ls7366rReadCounter();
  long guarded_encoder_count = updateEncoderFaultGuard(raw_encoder_count, millis());

  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    encoderCount = (int32_t)guarded_encoder_count;
  }
  printEncoderCsv(guarded_encoder_count);
  ...
  if (millis() - lp > 100) {
    ...
    Serial.print(" | EFG:");
    Serial.print(encoderGuardStatus);
    Serial.print(" rej:");
    Serial.print(encoderFaultRejects);
    Serial.print(" | TH:");
