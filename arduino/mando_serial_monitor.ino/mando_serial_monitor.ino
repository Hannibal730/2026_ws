...
#define LS7366R_CLEAR_CNTR 0x20
...
#define THROTTLE_MONITOR_INTERVAL_MS 100
...
void ls7366rClearCounter() {
  SPI.beginTransaction(ls7366rSpiSettings);
  digitalWrite(LS7366R_CS_PIN, LOW);
  SPI.transfer(LS7366R_CLEAR_CNTR);
  digitalWrite(LS7366R_CS_PIN, HIGH);
  SPI.endTransaction();
}
...
void printDeadbandMonitor(long throttle_pulse_us,
                          uint32_t throttle_last_pulse_us,
                          long steer_pulse_us,
                          uint32_t steer_last_pulse_us,
                          uint32_t now_us,
                          double steer_target_deg,
                          double steer_current_deg,
                          double steer_u) {
  static unsigned long previous_print_ms = 0;
  unsigned long now_ms = millis();

  if (now_ms - previous_print_ms < THROTTLE_MONITOR_INTERVAL_MS) return;
  previous_print_ms = now_ms;

  if (Serial.availableForWrite() < ENCODER_STREAM_MIN_TX_SPACE) return;

  const int throttle_deadband_low = ACCEL_CENTER_US - ACCEL_DB_US;
  const int throttle_deadband_high = ACCEL_CENTER_US + ACCEL_DB_US;
  bool throttle_signal_ok = (throttle_last_pulse_us != 0) &&
                            ((uint32_t)(now_us - throttle_last_pulse_us) <= 30000UL);

  int throttle_delta_us = 0;
  float throttle_normalized = 0.0f;
  const char *throttle_state = "SIGNAL_LOST";

  if (throttle_signal_ok) {
    throttle_delta_us = (int)throttle_pulse_us - ACCEL_CENTER_US;
    throttle_state = "DEADBAND";

    if (throttle_pulse_us > throttle_deadband_high) {
      throttle_normalized = (float)(throttle_pulse_us - ACCEL_CENTER_US) /
                            (float)(ACCEL_FWD_MAX_US - ACCEL_CENTER_US);
      throttle_state = "FORWARD";
    } else if (throttle_pulse_us < throttle_deadband_low) {
      throttle_normalized = (float)(throttle_pulse_us - ACCEL_CENTER_US) /
                            (float)(ACCEL_CENTER_US - ACCEL_REV_MIN_US);
  ...
  const int steer_deadband_low = STEER_CENTER_US - STEER_DB_US;
  const int steer_deadband_high = STEER_CENTER_US + STEER_DB_US;
  bool steer_signal_ok = (steer_last_pulse_us != 0) &&
                         ((uint32_t)(now_us - steer_last_pulse_us) <= 30000UL);

  int steer_delta_us = 0;
  float steer_normalized = 0.0f;
  const char *steer_pulse_state = "SIGNAL_LOST";

  if (steer_signal_ok) {
    steer_delta_us = (int)steer_pulse_us - STEER_CENTER_US;
    steer_normalized = normalizeSteerPulse(steer_pulse_us);
    steer_pulse_state = (steer_normalized == 0.0f) ? "DEADBAND" :
                        (steer_normalized > 0.0f ? "RIGHT" : "LEFT");
  }

  double steer_error_deg = steer_target_deg - steer_current_deg;
  const char *steer_angle_state = (fabs(steer_error_deg) < STEER_ERROR_DEADBAND_DEG) ?
                                  "DEADBAND" : "ACTIVE";
  const char *steer_pid_state = (fabs(steer_u) < PID_DEADBAND) ? "DEADBAND" : "ACTIVE";

  Serial.print("THDB,");
  if (throttle_signal_ok) {
    Serial.print(throttle_pulse_us);
  } else {
    Serial.print("SIGNAL_LOST");
  }
  Serial.print(",");
  Serial.print(throttle_delta_us);
  Serial.print(",");
  Serial.print(throttle_deadband_low);
  Serial.print(",");
  Serial.print(throttle_deadband_high);
  Serial.print(",");
  Serial.print(throttle_state);
  Serial.print(",");
  Serial.print(throttle_normalized, 3);
  Serial.print(",");
  if (steer_signal_ok) {
    Serial.print(steer_pulse_us);
  } else {
    Serial.print("SIGNAL_LOST");
  }
  Serial.print(",");
  Serial.print(steer_delta_us);
  Serial.print(",");
  Serial.print(steer_deadband_low);
  Serial.print(",");
  Serial.print(steer_deadband_high);
  Serial.print(",");
  Serial.print(steer_pulse_state);
  Serial.print(",");
  Serial.print(steer_normalized, 3);
  Serial.print(",");
  Serial.print(steer_target_deg, 2);
  Serial.print(",");
  Serial.print(steer_current_deg, 2);
  Serial.print(",");
  Serial.print(steer_error_deg, 2);
  Serial.print(",");
  Serial.print(STEER_ERROR_DEADBAND_DEG, 2);
  Serial.print(",");
  Serial.print(steer_angle_state);
  Serial.print(",");
  Serial.print(steer_u, 3);
  Serial.print(",");
  Serial.print(PID_DEADBAND, 2);
  Serial.print(",");
  Serial.println(steer_pid_state);
}
...
void setup() {
  ...
  Serial.println("THDB,throttle_pulse_us,throttle_delta_us,throttle_deadband_low,throttle_deadband_high,throttle_state,throttle_normalized,steer_pulse_us,steer_delta_us,steer_deadband_low,steer_deadband_high,steer_pulse_state,steer_normalized,steer_target_deg,steer_current_deg,steer_error_deg,steer_error_deadband_deg,steer_error_state,steer_pid_u,steer_pid_deadband,steer_pid_state");
}
...
void loop() {
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    ...
    steer_last_us_local = steer_last_us;
  }

  long raw_Accel_us_local = Accel_us_local;
  long raw_Steering_us_local = Steering_us_local;
  ...
  printDeadbandMonitor(raw_Accel_us_local,
                       accel_last_us_local,
                       raw_Steering_us_local,
                       steer_last_us_local,
                       t_us,
                       ref_steer_deg,
                       deg,
